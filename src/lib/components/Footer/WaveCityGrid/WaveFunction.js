...
export function collapseTiles(currentTiles, startRow, NUM_ROWS, NUM_COLS) {

    let tiles = currentTiles.slice(startRow * NUM_COLS, (startRow + 3) * NUM_COLS);
        ...
        if (minEntropyTileIndex != -1) {
            ...
            if (tileOptions.length !== 0) {
                ...
                tempTiles[minEntropyTileIndex] = {
                    order: (startRow * NUM_COLS) + iterations,
                    collapsed: true,
                    connections: {
                    ...
                    image: {
                        paths: chosenTileConfig.image.paths[Math.floor(Math.random() * chosenTileConfig.image.paths.length)],
                        rotation: chosenTileConfig.image.rotation[Math.floor(Math.random() * chosenTileConfig.image.rotation.length)]
                    },
                    entropy: 0,
                    color: "TBD"
                }
    ...
    let resultTiles = [...currentTiles.slice(0, startRow * NUM_COLS), ...tiles];
    return resultTiles;
}
...
export function initTiles(currentTiles, NUM_ROWS, NUM_COLS, rowsToAdd = 2) {
    let numNewTiles = rowsToAdd * NUM_COLS;
    let newTiles = Array.from({ length: numNewTiles }, () => ({}));
    ...
    let resultTiles = [...currentTiles, ...newTiles];
